...
def prune_unlisted_indices(root_dir: Path, keep_targets: set, apply: bool) -> List[str]:
    removed: List[str] = []
    for p in root_dir.rglob("_index.md"):
        try:
            fm, _ = read_frontmatter_and_body(p)
            if str(fm.get("managed_by", "")).strip() != MANAGED_BY:
                continue
            if p not in keep_targets:
                if apply:
...
def main(argv: Optional[List[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="categories.csv → Hugo _index.md (DE+EN)")
    ap.add_argument("--csv", default="ssot/categories.csv", help="Pfad zur categories.csv (relativ zu wissen/)")
    ap.add_argument("--de-root", default="content/de", help="DE content root (relativ zu wissen/)")
    ap.add_argument("--en-root", default="content/en", help="EN content root (relativ zu wissen/)")
    ap.add_argument("--apply", action="store_true", help="Schreibt Dateien (sonst Dry-Run)")
    ap.add_argument("--prune", action="store_true", help="Entfernt verwaltete _index.md, die nicht mehr in categories.csv vorkommen")
    ap.add_argument("--report", default=None, help="Optionaler Report-Pfad (Markdown)")
    ...
    if not csv_path.exists():
        print(f"[ERROR] categories.csv not found: {csv_path}", file=sys.stderr)
        return 2

    raw_rows = read_csv_utf8_auto(csv_path)
    cats: List[CategoryRow] = [row_to_category(r) for r in raw_rows if clean(r.get("key"))]

    errs = validate_categories(cats)
    if errs:
        print("[ERROR] categories.csv validation failed:", file=sys.stderr)
        for e in errs[:200]:
            print(f"  - {e}", file=sys.stderr)
        if len(errs) > 200:
            print(f"  ... ({len(errs) - 200} more)", file=sys.stderr)
        return 2
    ...
    for c in cats:
        # DE
        target_de = de_root / c.path_de.strip("/") / "_index.md"
        fm_de, body_de = build_frontmatter(c, "de", now_utc)
        res = write_index(target_de, fm_de, body_de, apply=args.apply)
        keep_targets.add(target_de)
        if res == "created":
            created.append(str(target_de))
        elif res == "updated":
            updated.append(str(target_de))
        elif res == "unchanged":
            unchanged.append(str(target_de))
        else:
            would.append(str(target_de))

        # EN
        target_en = en_root / c.path_en.strip("/") / "_index.md"
        fm_en, body_en = build_frontmatter(c, "en", now_utc)
        res = write_index(target_en, fm_en, body_en, apply=args.apply)
        keep_targets.add(target_en)
        if res == "created":
            created.append(str(target_en))
        elif res == "updated":
            updated.append(str(target_en))
        elif res == "unchanged":
            unchanged.append(str(target_en))
        else:
            would.append(str(target_en))

    if args.prune:
        pruned += prune_unlisted_indices(de_root, keep_targets, apply=args.apply)
        pruned += prune_unlisted_indices(en_root, keep_targets, apply=args.apply)

    ts = now_utc.strftime("%Y%m%d-%H%M%S")
    report_rel = Path(args.report) if args.report else Path("scripts/reports") / f"categories-sync-{ts}.md"
    report_path = (repo_wissen / report_rel).resolve()
    ...
    rep_parts: List[str] = []
    rep_parts.append(f"# Categories Sync Report ({ts})\n")
    rep_parts.append(f"- CSV: {csv_path}\n- DE root: {de_root}\n- EN root: {en_root}\n- Apply: {args.apply}\n- Prune: {args.prune}\n")
    rep_parts.append(_sec("Written (created/updated)", created + updated))
    rep_parts.append(_sec("Unchanged", unchanged))
    rep_parts.append(_sec("Would write (dry-run)", would))
    rep_parts.append(_sec("Pruned (managed_by=categories.csv)", pruned))

    report_path.write_text("\n".join([x for x in rep_parts if x]).strip() + "\n", encoding="utf-8")
    print(f"[OK] Categories report: {report_path}")
    return 0
